...
const char * const BITCOIN_CONF_FILENAME = "vexxacoin.conf";
const char * const BITCOIN_PID_FILENAME = "vexxacoin.pid";
...
#else
    const char* pszModule = "vexxacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vexxacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Vexxacoin";
#else
    // Unix
    return pathRet / ".vexxacoin";
#endif
#endif
}
